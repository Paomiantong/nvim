...
local get_workspace_folders = require('internal.whichpy.locator._common').get_workspace_folders
...
function Locator.new(opts)
  local obj = vim.tbl_deep_extend('force', {
    ...
    search_pattern = '.*env.*',
    depth = 2,
    ignore_dirs = { '.git', '.mypy_cache', '.pytest_cache', '.ruff_cache', '__pycache__', '__pypackages__' },
  }, opts or {})
...
function Locator:find()
  ...
  return coroutine.wrap(function()
    while #dirs > 0 do
      local item = table.remove(dirs, 1)
